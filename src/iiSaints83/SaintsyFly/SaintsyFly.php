	...
	private function multiWorldCheck(Entity $entity) : bool{
		...
		if($this->getConfig()->get("multi-world") === "on"){
			if(!in_array($entity->getLevel()->getName(), $this->getConfig()->get("worlds"))){
				$entity->sendMessage(self::PREFIX . TextFormat::RED . "You are not in the correct world to be able to use the fly command");
		...
		if($command->getName() === "fly"){
			...
			if(empty($args[0])){
				if(!$sender->isCreative()){
					...
					$sender->sendMessage(self::PREFIX . TextFormat::AQUA . "You can only use this command in survival mode");
					return false;
				}
				return false;
			}
			if(!$sender->hasPermission("blazinfly.other")){
				$sender->sendMessage(self::PREFIX . TextFormat::AQUA . "You do not have permission to enable flight for others");
				return false;
			}
			...
			if($event instanceof EntityDamageByEntityEvent){
				if($entity instanceof Player){
					...
					if($damager->getAllowFlight() === true){
						$damager->sendMessage(self::PREFIX . TextFormat::LIGHT_BLUE . "Flight disabled due to combat");
